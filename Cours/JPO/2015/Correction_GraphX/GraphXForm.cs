...
namespace GraphX
{
    public partial class GraphXForm : Form
    {
        ...
        private Point derivative = new Point(1, 1);
        ...
        {
            temporaryEdge = new Edge(selectedVertex, new Vertex(destination, true));
        }
        ...
        {
            Vertex other = CheckWhoIntersects(new Vertex(e.Location, true));

            if (other == null)
            {
                AddVertex(new Vertex(e.Location, false));
            }
            else
            {
        ...
        private void notasecret()
        {
            Control[] labels = mainPanel.Controls.Find("labelDoge", true);
            if (labels.Length == 0)
            {
                Timer timer = new Timer();
                timer.Interval = 16;
                timer.Tick += timer_Tick;
                Label labelDoge = new Label();
                labelDoge.Image = global::GraphX.Properties.Resources.doge;
                labelDoge.Location = new System.Drawing.Point(60, 40);
                labelDoge.Name = "labelDoge";
                labelDoge.Size = new System.Drawing.Size(300, 218);
                mainPanel.Controls.Add(labelDoge);
                labelDoge.Show();
        ...
        private void timer_Tick(object sender, EventArgs e)
        {
            ...
            if (labels.Length > 0)
            {
                Label labelDoge = (Label)labels[0];
                if (labelDoge.Location.X <= 0)
                {
                    derivative.X = 1;
                }
                if (labelDoge.Location.X + labelDoge.Size.Width >= mainPanel.Size.Width - separatorPanel.Width)
                {
                    derivative.X = -1;
                }
                if (labelDoge.Location.Y <= 0)
                {
                    derivative.Y = 1;
                }
                if (labelDoge.Location.Y + labelDoge.Size.Height >= mainPanel.Size.Height - horizontalScrollBar.Height)
                {
                    derivative.Y = -1;
                }

                labelDoge.Location = new Point(labelDoge.Location.X + derivative.X, labelDoge.Location.Y + derivative.Y);
            }
        }
