...
namespace New_JPO
{
    public partial class JPO : Form
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            // On parcourt tous les blocs contenus dans le tableau pour connaître ceux touchés par la balle
            for (int i = 0; i < Constantes.NB_BLOCS_HAUTEUR; i++)
                for (int j = 0; j < Constantes.NB_BLOCS_LARGEUR; j++)
                {
                    if (!balle.toucheBloc)    // Si la balle n'a pas encore touché de bloc
                    {
                         bool haut = false, bas = false, droite = false, gauche = false; // On détermine si le bloc à des blocs voisins
                         if (i > 0)
                             if (blocs[i - 1][j].Visible)
                                 gauche = true;
                         if (i < Constantes.NB_BLOCS_HAUTEUR - 1)
                             if (blocs[i + 1][j].Visible)
                                 droite = true;
                         if (j > 0)
                             if (blocs[i][j - 1].Visible)
                                 haut = true;
                         if (j < Constantes.NB_BLOCS_LARGEUR - 1)
                             if (blocs[i][j + 1].Visible)
                                 bas = true;

                         score += balle.toucheDuBloc(blocs[i][j], haut, bas, gauche, droite); // fonction qui retourne le nombre de point gagné si le bloc est touché, 0 sinon
                    }
                }
